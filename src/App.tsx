  ...
  return (
    <div ref={appRef} className="portfolio-app-root">
      ...
      <header className="site-header-fixed">
        ...
        {/* Far-right Desktop Vertical Nav Menu */}
        ...
        {/* Mobile / iPhone Collapsible Menu Trigger Button */}
        <button
          onClick={() => setIsMobileMenuOpen(prev => !prev)}
          className="mobile-menu-pill-btn"
          aria-label={isMobileMenuOpen ? "Close navigation menu" : "Open navigation menu"}
          aria-expanded={isMobileMenuOpen}
          aria-controls="mobileNavModalBackdrop"
        >
          <span className="mobile-menu-active-badge font-mono">
            {activeSectionIdx === 0 ? 'HOME' : `0${activeSectionIdx} ${SECTIONS_CONFIG[activeSectionIdx]?.name.toUpperCase()}`}
          </span>
      ...
      {/* ─── MOBILE / IPHONE NAVIGATION DRAWER MODAL ─── */}
      <div
        id="mobileNavModalBackdrop"
        className={`mobile-nav-modal-backdrop ${isMobileMenuOpen ? 'is-open' : ''}`}
        aria-hidden={!isMobileMenuOpen}
        onClick={(e) => {
          if (e.target === e.currentTarget) setIsMobileMenuOpen(false);
        }}
      >
        <div className="mobile-nav-drawer-sheet">
          <div className="mobile-nav-sheet-header">
            ...
            <button
              ...
              className="mobile-nav-sheet-close-btn font-mono"
          ...
          <nav className="mobile-nav-links-list" aria-label="Mobile Navigation">
            {SECTIONS_CONFIG.map((sec, idx) => {
              ...
              return (
                ...
                >
                  <span className="mobile-item-num font-mono">0{idx}</span>
                  <span className="mobile-item-name">{sec.name}</span>
                  {isActive && <span className="mobile-item-dot" aria-hidden="true" />}
                </button>
              );
            })}
          </nav>
          ...
          <div className="mobile-nav-sheet-footer">
            <div className="mobile-footer-actions">
              <a
                ...
                className="mobile-footer-pill-btn font-mono"
              >
                <span>DOWNLOAD CV</span>
                <span aria-hidden="true">↓</span>
              </a>
              <a
                href="https://devxin.net"
                target="_blank"
