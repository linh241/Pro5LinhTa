...
export interface DitheredObjectOptions {
  ...
  yOffset?: number;
  /** Static model rotation X in radians. */
  rotationX?: number;
  /** Static model rotation Y in radians. */
  rotationY?: number;
  /** Static model rotation Z in radians. */
  rotationZ?: number;
...
export function createDitheredObject(
  ...
  function buildRoom() {
    ...
    for (const [x, z] of [
      ...
      const spot = new THREE.SpotLight(config.highlight || 0xffffff, 2, 0, 0.2, 1, 0);
      spot.position.set(x, 20, z);
      room.add(spot, spot.target);
    }
    const center = new THREE.PointLight(config.highlight || 0xffffff, 100, 28, 2);
    ...
    for (const def of ROOM_FORMERS) {
      ...
      room.add(mesh);
      if (def.withLight) {
        const light = new THREE.PointLight(config.highlight || 0xffffff, 100, 28, 2);
  ...
  function applyFit() {
    ...
    fitGroup.rotation.set(config.rotationX, config.rotationY, config.rotationZ);
  }
  ...
  function tick(time: number) {
    ...
    if (!reducedMotion) {
      ...
      floatGroup.rotation.x = (Math.cos(elapsed / 3) / 12) * config.rotationIntensity;
      floatGroup.rotation.y = (Math.sin(elapsed / 4) / 10) * config.rotationIntensity;
      floatGroup.rotation.z = Math.sin(elapsed * 2.2) * 0.28 * config.rotationIntensity;
      floatGroup.position.y =
        MODEL_LIFT +
        config.yOffset +
        (Math.sin(elapsed / 1.5) / 12) * config.floatIntensity;
    }
